...
PROCESS = re.compile(r"^Single outlet(?=[,.;:]| (only|so far|among)\b)|^We read\b|\bamong the records here\b|\bthis page (rests|holds|carries|shows|treats|does|did|could|can|cannot|uses|cites|relies|counts|reads|lists|names|includes|leaves|reports|says|finds|found|has|is|was|labels|checks|compares|confirms|computes|tracks|covers|quotes)\b|\bon this page\b|\bthe page (found|could|did|does)\b|\bthis story rests on\b|\b(what )?the (records?|coverage|numbers) (here )?adds? up to\b|\bwe could( not)? (preserve|save|fetch|reach|obtain|get|find|locate)\b|\bas reproduced by\b|\b(did|does) not answer through\b|\bnot available through the (records|registry|archive|pins|admitted|accessible)\b|^Searched:|\bthe (accessible|admitted) (filings|records|sources)\b|\bpublic sources we\b", re.I)
