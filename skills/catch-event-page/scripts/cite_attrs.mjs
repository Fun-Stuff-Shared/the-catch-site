...
// record id and the passage as literals: "...", '...', {"..."}, or {`...`} with no interpolation
// (the single-quote and backtick forms are for a passage that itself contains double quotes).
// Any other form (a spread, an expression, a brace around single quotes, a space around the
// equals sign) is unreadable, and a lint that cannot read a Cite refuses it rather than
...
export function readCites(source) {
  ...
  for (const m of source.matchAll(citeTag)) {
    ...
    if (cite.readable) {
      for (const a of attrs.matchAll(one)) cite[a[1]] = a[2] ?? a[3] ?? a[4] ?? a[5];
