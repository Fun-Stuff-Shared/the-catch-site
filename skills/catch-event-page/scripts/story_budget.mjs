...
const tableLines = gradesBlock[1].split("\n").filter((line) => /^\s*\|/.test(line));
for (const [i, line] of tableLines.entries()) {
  const cells = cellsOf(line);
  if (isSeparator(cells)) continue;
  if (i + 1 < tableLines.length && isSeparator(cellsOf(tableLines[i + 1]))) continue;
  const idm = cells.length === 4 ? cells[0].match(/^`([^`]+)`$/) : null;
  if (!idm || !/^[ABCD]$/.test(cells[2])) {
    defects.push(`${model}: Grades row the lint cannot read; a row is four cells, \`record id\` | passage | A to D | serves, with a pipe inside a cell written \\|: ${line.trim().slice(0, 100)}`);
    continue;
  }
...
if (rows.length === 0 && defects.length === 0) { console.error(`story_budget: no graded rows under "## Grades" in ${model}`); process.exit(2); }
...
const { text: source, proofRanges } = renderedView(readFileSync(page, "utf8"));
...
// block. A Cite under any proof element does not carry it.
