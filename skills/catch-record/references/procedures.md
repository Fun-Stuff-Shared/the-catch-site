...
- reuters.com, nytimes.com and wsj.com are credentialed sources in the registry (`capture credentials` lists them with their cookie files): `capture news <url>` serves their article bodies, and the six-hourly wire sweep already holds most Reuters and Times pieces from the last days, so run `capture search "<slug words>"` first and pin the held text. A refusal from any of the three is a stale cookie file (`capture credentials` shows the age; `scripts/connectors/refresh_browser_cookies.py --source <id>` in news-fqs-pilot refreshes it from the logged-in browser), not a blocked host; report it as such.
- Blocked in every mode: cbo.gov (403), congress.gov (challenge), courts.mo.gov opinion PDFs, house.mo.gov, spglobal, ft.com (no credential file yet). archive.org had no snapshot for any CBO page tried.
- Carrier copies carry the same text for the hosts still blocked: AP via KPBS, NPR, PBS. Pin the carrier, name the carrier in the records list and in the ledger's recovery paragraph, quote the carrier's bytes.
